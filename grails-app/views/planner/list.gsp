							...
							<table>
								...
								<g:each in="${plannerInstanceList}" status="i" var="plannerInstance">
									<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
										...
										<td>${fieldValue(bean: plannerInstance.user, field: "firstName")}</td>
										<td>${fieldValue(bean: plannerInstance.user, field: "lastName")}</td>
										<td>${fieldValue(bean: plannerInstance.user, field: "email")}</td>
									</tr>
								</g:each>
								</tbody>
							</table>
							<div class="pagination">
								<g:paginate total="${plannerInstanceTotal}" />
							</div>
						</div>									
					</div>
